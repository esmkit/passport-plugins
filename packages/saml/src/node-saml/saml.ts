...
import { AuthenticateOptions, AuthorizeOptions, Profile, SamlConfig, ErrorWithXmlStatus } from "../types";
...
class SAML {
  ...
  initialize(ctorOptions: SamlConfig): SamlOptions {
    ...
    const options = {
      ...
      authnContext: ctorOptions.authnContext ?? ["urn:oasis:names:tc:SAML:2.0:ac:classes:PasswordProtectedTransport"],
  ...
  async getAuthorizeUrlAsync(RelayState: string, host: string | undefined, options: AuthorizeOptions): Promise<string> {
  ...
  async getAuthorizeFormAsync(RelayState: string, host?: string): Promise<string> {
    ...
    const quoteattr = function (
      s: string | number | boolean | undefined | null | readonly string[] | readonly number[] | readonly boolean[],
      preserveCR?: boolean
    ) {
  ...
  async getLogoutUrlAsync(user: Profile, RelayState: string, options: AuthenticateOptions & AuthorizeOptions) {
  ...
  validateSignature(fullXml: string, currentNode: Element, certs: string[]): boolean {
    ...
    return certs.some(certToCheck => {
      return validateXmlSignatureForCert(signature, this._certToPEM(certToCheck), fullXml, currentNode);
    });
  }
  ...
  async validatePostResponseAsync(
    ...
    try {
      ...
      if (
        ...
        Array.from(doc.childNodes as NodeListOf<Element>).filter(n => n.tagName != null && n.childNodes != null)
          .length === 1
      ) {
        validSignature = true;
      }
  ...
  async validateRedirectAsync(
    ...
    samlMessageType === "SAMLResponse" ? await this.verifyLogoutResponse(doc) : this.verifyLogoutRequest(doc);
  ...
  private validateSignatureForRedirect(urlString: crypto.BinaryLike, signature: string, alg: string, cert: string) {
  ...
  private async processValidlySignedAssertionAsync(xml: string, samlResponseXml: string, inResponseTo: string) {
    let msg;
    const nowMs = new Date().getTime();
    const profile = {} as Profile;
    ...
    getInResponseTo: {
      ...
      if (subject) {
        ...
        subjectConfirmation = subject[0].SubjectConfirmation ? subject[0].SubjectConfirmation[0] : null;
        confirmData =
  ...
  async _getNameIdAsync(self: SAML, doc: Node): Promise<NameID> {
    ...
    if (encryptedIds.length === 1) {
      ...
      const encryptedDatas = xpath.selectElements(encryptedIds[0], "./*[local-name()='EncryptedData']");
  ...
  _keyToPEM(key: string | Buffer): typeof key extends string | Buffer ? string | Buffer : Error {
    ...
    if (matchedKey) {
      const wrappedKey = ["-----BEGIN PRIVATE KEY-----", ...matchedKey, "-----END PRIVATE KEY-----", ""].join("\n");
      return wrappedKey;
    }
